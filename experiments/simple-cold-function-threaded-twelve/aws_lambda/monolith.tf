...
resource "aws_lambda_function" "monolith-python" {
  ...
  function_name = "simple-cold-function-threaded-twelve-monolith"
  ...
  publish = true
  layers = [aws_lambda_layer_version.simple-cold-function-threaded-twelve-monolith-lambda-layer.arn]
  timeout = 60
}
