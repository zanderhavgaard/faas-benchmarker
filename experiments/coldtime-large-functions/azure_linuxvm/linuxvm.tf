# create linux vm
resource "azurerm_linux_virtual_machine" "coldtime-large-functions-worker" {
  depends_on = [azurerm_network_interface.coldtime-large-functions-worker-ni]
  ...
  network_interface_ids = [
    azurerm_network_interface.coldtime-large-functions-worker-ni.id,
  ]
...
resource "null_resource" "linux-provisioners" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.coldtime-large-functions-worker,
    azurerm_public_ip.coldtime-large-functions-worker-public-ip
  ]

  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    host = azurerm_linux_virtual_machine.coldtime-large-functions-worker.public_ip_address
    type = "ssh"
