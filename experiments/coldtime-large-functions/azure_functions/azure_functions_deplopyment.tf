...
resource "azurerm_resource_group" "coldtime-large-functions-rg" {
  name = "coldtime-large-functions-rg"
  location = var.azure_region
}

# create service plan for rg
resource "azurerm_app_service_plan" "coldtime-large-functions-plan" {
  name = "coldtime-large-functions-plan"
  location = var.azure_region
...
resource "azurerm_storage_account" "coldtime-large-functions-experiment-storage" {
...
resource "azurerm_storage_container" "coldtime-large-functions-container" {
  name = "coldtime-large-functions-container"
...
data "azurerm_storage_account_sas" "sas-coldtime-large-functions" {
  connection_string = azurerm_storage_account.coldtime-large-functions-experiment-storage.primary_connection_string
  https_only        = false
  resource_types {
    service   = false
    container = false
...
resource "azurerm_application_insights" "coldtime-large-functions" {
  name = "coldtime-large-functions-insights"
  location = var.azure_region
  resource_group_name = azurerm_resource_group.coldtime-large-functions-rg.name
