# create linux vm
resource "azurerm_linux_virtual_machine" "linear-invocation-worker" {
  depends_on = [azurerm_network_interface.linear-invocation-worker-ni]
  ...
  network_interface_ids = [
    azurerm_network_interface.linear-invocation-worker-ni.id,
  ]
...
resource "null_resource" "linux-provisioners" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.linear-invocation-worker,
    azurerm_public_ip.linear-invocation-worker-public-ip
  ]

  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    host = azurerm_linux_virtual_machine.linear-invocation-worker.public_ip_address
    type = "ssh"
