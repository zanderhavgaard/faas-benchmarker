# create linux vm
resource "azurerm_linux_virtual_machine" "function-lifetime-openfaas-worker" {
  depends_on = [azurerm_network_interface.function-lifetime-openfaas-worker-ni]
  ...
  network_interface_ids = [
    azurerm_network_interface.function-lifetime-openfaas-worker-ni.id,
  ]
...
resource "null_resource" "linux-provisioners" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.function-lifetime-openfaas-worker,
    azurerm_public_ip.function-lifetime-openfaas-worker-public-ip
  ]

  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    host = azurerm_linux_virtual_machine.function-lifetime-openfaas-worker.public_ip_address
    type = "ssh"
