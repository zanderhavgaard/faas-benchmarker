...
resource "azurerm_resource_group" "function-lifetime-rg" {
  name = "function-lifetime-rg"
  location = var.azure_region
}

# create service plan for rg
resource "azurerm_app_service_plan" "function-lifetime-plan" {
  name = "function-lifetime-plan"
  location = var.azure_region
...
resource "azurerm_storage_account" "function-lifetime-experiment-storage" {
...
resource "azurerm_storage_container" "function-lifetime-container" {
  name = "function-lifetime-container"
...
data "azurerm_storage_account_sas" "sas-function-lifetime" {
  connection_string = azurerm_storage_account.function-lifetime-experiment-storage.primary_connection_string
  https_only        = false
  resource_types {
    service   = false
    container = false
...
resource "azurerm_application_insights" "function-lifetime" {
  name = "function-lifetime-insights"
  location = var.azure_region
  resource_group_name = azurerm_resource_group.function-lifetime-rg.name
