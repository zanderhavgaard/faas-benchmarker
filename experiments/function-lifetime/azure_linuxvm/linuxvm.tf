# create linux vm
resource "azurerm_linux_virtual_machine" "function-lifetime-worker" {
  depends_on = [azurerm_network_interface.function-lifetime-worker-ni]
  ...
  network_interface_ids = [
    azurerm_network_interface.function-lifetime-worker-ni.id,
  ]
...
resource "null_resource" "linux-provisioners" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.function-lifetime-worker,
    azurerm_public_ip.function-lifetime-worker-public-ip
  ]

  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    host = azurerm_linux_virtual_machine.function-lifetime-worker.public_ip_address
    type = "ssh"
