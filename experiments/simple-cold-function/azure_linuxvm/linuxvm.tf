# create linux vm
resource "azurerm_linux_virtual_machine" "simple-cold-function-worker" {
  depends_on = [azurerm_network_interface.simple-cold-function-worker-ni]
  ...
  network_interface_ids = [
    azurerm_network_interface.simple-cold-function-worker-ni.id,
  ]
...
resource "null_resource" "linux-provisioners" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.simple-cold-function-worker,
    azurerm_public_ip.simple-cold-function-worker-public-ip
  ]

  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    host = azurerm_linux_virtual_machine.simple-cold-function-worker.public_ip_address
    type = "ssh"
