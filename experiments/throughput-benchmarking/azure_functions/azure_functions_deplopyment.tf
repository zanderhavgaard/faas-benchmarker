...
resource "azurerm_resource_group" "throughput-benchmarking-rg" {
  name = "throughput-benchmarking-rg"
  location = var.azure_region
}

# create service plan for rg
resource "azurerm_app_service_plan" "throughput-benchmarking-plan" {
  name = "throughput-benchmarking-plan"
  location = var.azure_region
...
resource "azurerm_storage_account" "throughput-benchmarking-experiment-storage" {
...
resource "azurerm_storage_container" "throughput-benchmarking-container" {
  name = "throughput-benchmarking-container"
...
data "azurerm_storage_account_sas" "sas-throughput-benchmarking" {
  connection_string = azurerm_storage_account.throughput-benchmarking-experiment-storage.primary_connection_string
  https_only        = false
  resource_types {
    service   = false
    container = false
...
resource "azurerm_application_insights" "throughput-benchmarking" {
  name = "throughput-benchmarking-insights"
  location = var.azure_region
  resource_group_name = azurerm_resource_group.throughput-benchmarking-rg.name
