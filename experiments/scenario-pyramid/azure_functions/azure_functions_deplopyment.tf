...
resource "azurerm_resource_group" "scenario-pyramid-rg" {
  name = "scenario-pyramid-rg"
  location = var.azure_region
}

# create service plan for rg
resource "azurerm_app_service_plan" "scenario-pyramid-plan" {
  name = "scenario-pyramid-plan"
  location = var.azure_region
...
resource "azurerm_storage_account" "scenario-pyramid-experiment-storage" {
...
resource "azurerm_storage_container" "scenario-pyramid-container" {
  name = "scenario-pyramid-container"
...
data "azurerm_storage_account_sas" "sas-scenario-pyramid" {
  connection_string = azurerm_storage_account.scenario-pyramid-experiment-storage.primary_connection_string
  https_only        = false
  resource_types {
    service   = false
    container = false
...
resource "azurerm_application_insights" "scenario-pyramid" {
  name = "scenario-pyramid-insights"
  location = var.azure_region
  resource_group_name = azurerm_resource_group.scenario-pyramid-rg.name
