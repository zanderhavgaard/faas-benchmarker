# create linux vm
resource "azurerm_linux_virtual_machine" "time-to-cold-start-twelve-threads-openfaas-worker" {
  depends_on = [azurerm_network_interface.time-to-cold-start-twelve-threads-openfaas-worker-ni]
  ...
  network_interface_ids = [
    azurerm_network_interface.time-to-cold-start-twelve-threads-openfaas-worker-ni.id,
  ]
...
resource "null_resource" "linux-provisioners" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.time-to-cold-start-twelve-threads-openfaas-worker,
    azurerm_public_ip.time-to-cold-start-twelve-threads-openfaas-worker-public-ip
  ]

  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    host = azurerm_linux_virtual_machine.time-to-cold-start-twelve-threads-openfaas-worker.public_ip_address
    type = "ssh"
