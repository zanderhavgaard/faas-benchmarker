...
data "archive_file" "time-to-cold-start-twelve-threads3-function-code" {
  type = "zip"
  source_dir = "function_code/time-to-cold-start-twelve-threads-function3"
  output_path = "function3.zip"
}

# upload zip archive to storage contianer
resource "azurerm_storage_blob" "time-to-cold-start-twelve-threads3-code" {
  name = "time-to-cold-start-twelve-threads3-function.zip"
...
resource "azurerm_function_app" "time-to-cold-start-twelve-threads3" {
  depends_on = [azurerm_storage_blob.time-to-cold-start-twelve-threads3-code]
  ...
  app_settings = {
    HASH = data.archive_file.time-to-cold-start-twelve-threads3-function-code.output_base64sha256
    WEBSITE_RUN_FROM_PACKAGE = "${azurerm_storage_blob.time-to-cold-start-twelve-threads3-code.url}${data.azurerm_storage_account_sas.sas-time-to-cold-start-twelve-threads.sas}"
