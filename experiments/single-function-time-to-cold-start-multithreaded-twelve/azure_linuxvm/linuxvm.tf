# create linux vm
resource "azurerm_linux_virtual_machine" "single-function-time-to-cold-start-multithreaded-twelve-worker" {
  depends_on = [azurerm_network_interface.single-function-time-to-cold-start-multithreaded-twelve-worker-ni]
  ...
  network_interface_ids = [
    azurerm_network_interface.single-function-time-to-cold-start-multithreaded-twelve-worker-ni.id,
  ]
...
resource "null_resource" "linux-provisioners" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.single-function-time-to-cold-start-multithreaded-twelve-worker,
    azurerm_public_ip.single-function-time-to-cold-start-multithreaded-twelve-worker-public-ip
  ]

  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    host = azurerm_linux_virtual_machine.single-function-time-to-cold-start-multithreaded-twelve-worker.public_ip_address
    type = "ssh"
