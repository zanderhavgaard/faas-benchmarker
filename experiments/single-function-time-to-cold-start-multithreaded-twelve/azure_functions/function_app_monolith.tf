...
resource "azurerm_storage_blob" "single-function-time-to-cold-start-multithreaded-twelve-monolith-code" {
  name = "single-function-time-to-cold-start-multithreaded-twelve-monolith-function.zip"
...
resource "azurerm_function_app" "single-function-time-to-cold-start-multithreaded-twelve-monolith" {
  ...
  app_settings = {
    HASH = data.archive_file.single-function-time-to-cold-start-multithreaded-twelve-monolith-function-code.output_base64sha256
    WEBSITE_RUN_FROM_PACKAGE = "${azurerm_storage_blob.single-function-time-to-cold-start-multithreaded-twelve-monolith-code.url}${data.azurerm_storage_account_sas.sas-single-function-time-to-cold-start-multithreaded-twelve.sas}"
...
resource "azurerm_template_deployment" "single-function-time-to-cold-start-multithreaded-twelve-monolith-function-key" {
  depends_on = [azurerm_function_app.single-function-time-to-cold-start-multithreaded-twelve-monolith]

  name = "single-function-time-to-cold-start-multithreaded-twelve-monolith_get_function_key"
  parameters = {
    "functionApp" = azurerm_function_app.single-function-time-to-cold-start-multithreaded-twelve-monolith.name
  }
  resource_group_name    = azurerm_resource_group.single-function-time-to-cold-start-multithreaded-twelve-rg.name
...
output "single-function-time-to-cold-start-multithreaded-twelve-monolith_function_key" {
  value = "${lookup(azurerm_template_deployment.single-function-time-to-cold-start-multithreaded-twelve-monolith-function-key.outputs, "functionkey")}"
}
output "single-function-time-to-cold-start-multithreaded-twelve-monolith_function_app_url" {
  value = azurerm_function_app.single-function-time-to-cold-start-multithreaded-twelve-monolith.default_hostname
}
