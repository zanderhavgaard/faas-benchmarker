...
def invoke():
    if(thread_numb == 1):
        ...
        invocation_list = filter(None, [x if 'error' not in x else errors.append(x) for x in map(lambda x: lib.get_dict(
...
try:
    ...
    coldtime = initial_cold_start_response['execution_start'] - \
        initial_cold_start_response['invocation_start']
    ...
    avg_warmtime = validate(lib.reduce_dict_by_keys, 'avg_warmtime',
                            (create_invocation_list(), ('execution_start', 'invocation_start')))

    if(dev_mode):
        benchmark = coldtime * 0.98
        avg_warmtime = validate(lib.reduce_dict_by_keys, 'avg_warmtime', (create_invocation_list(
            (10, 'create invocation_list')), ('execution_start', 'invocation_start')))
        lib.dev_mode_print('Values before any checks -> coldtime xp', [(
            'coldtime', coldtime), ('benchmark', benchmark), ('avg_warmtime', avg_warmtime)])
    ...
    def check_coldtime(sleep: int):
        ...
        if(avg_warmtime < benchmark):
            ...
            coldtime = initial_cold_start_response['execution_start'] - \
                initial_cold_start_response['invocation_start']
            benchmark = coldtime * 0.90
            avg_warmtime = lib.wrappped_reduce_dict_by_keys('avg_warmtime', experiment_name, (
                create_invocation_list(), ('execution_start', 'invocation_start')), err_func)
            if(avg_warmtime > benchmark):
    ...
    def set_cold_values():
        ...
        while(increment > granularity):
            ...
            latest_latency_time = result_dict['execution_start'] - \
                result_dict['invocation_start']

            if(dev_mode):
                lib.dev_mode_print('logging WARM time coldtime exp', [
                    ('experiment_uuid,result_dict[identifier]',
                     experiment_uuid, result_dict['identifier']),
                    ('sleep_time / 60',
                     int(sleep_time / 60)),
                    ('sleep_time % 60', int(
    ...
    for i in range(5):
        ...
        latency_time = result_dict['execution_start'] - \
            result_dict['invocation_start']
        if(dev_mode):
