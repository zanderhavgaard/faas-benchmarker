# ec2 instance
resource "aws_instance" "concurrency-benchmarking1-worker-aws" {
  tags = {
    Name = "concurrency-benchmarking1-worker"
  }
...
resource "null_resource" "ec2-provisioners" {
  ...
  depends_on = [
    aws_eip.concurrency-benchmarking1-worker-eip,
    aws_instance.concurrency-benchmarking1-worker-aws
  ]
  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    # host = aws_instance.experiment-worker-aws.public_ip
    host = aws_eip.concurrency-benchmarking1-worker-eip.public_ip
    type = "ssh"
