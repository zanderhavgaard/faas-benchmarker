# create linux vm
resource "azurerm_linux_virtual_machine" "concurrency-benchmarking-worker" {
  depends_on = [azurerm_network_interface.concurrency-benchmarking-worker-ni]
  ...
  network_interface_ids = [
    azurerm_network_interface.concurrency-benchmarking-worker-ni.id,
  ]
...
resource "null_resource" "linux-provisioners" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.concurrency-benchmarking-worker,
    azurerm_public_ip.concurrency-benchmarking-worker-public-ip
  ]

  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    host = azurerm_linux_virtual_machine.concurrency-benchmarking-worker.public_ip_address
    type = "ssh"
