...
resource "azurerm_resource_group" "concurrency-benchmarking-rg" {
  name = "concurrency-benchmarking-rg"
  location = var.azure_region
}

# create service plan for rg
resource "azurerm_app_service_plan" "concurrency-benchmarking-plan" {
  name = "concurrency-benchmarking-plan"
  location = var.azure_region
...
resource "azurerm_storage_account" "concurrency-benchmarking-experiment-storage" {
...
resource "azurerm_storage_container" "concurrency-benchmarking-container" {
  name = "concurrency-benchmarking-container"
...
data "azurerm_storage_account_sas" "sas-concurrency-benchmarking" {
  connection_string = azurerm_storage_account.concurrency-benchmarking-experiment-storage.primary_connection_string
  https_only        = false
  resource_types {
    service   = false
    container = false
...
resource "azurerm_application_insights" "concurrency-benchmarking" {
  name = "concurrency-benchmarking-insights"
  location = var.azure_region
  resource_group_name = azurerm_resource_group.concurrency-benchmarking-rg.name
