...
def invoke(thread_numb:int):
    ...
    benchmarker.invoke_function_conccurrently(function_name=fx, numb_threads=thread_numb,function_args={'throughput_time':0.3}))]))
...
def append_result(dict:dict, err:int, thread_numb:int) -> None:
    results.append({
        ...
        'sleep': coldtime,
        'errors': err,
        'acc_throughput_time': dict['throughput_time'],
        'acc_throughput': 0 if dict['throughput_time'] == 0.0 else int(dict['throughput']),
        'acc_throughput_process_time': 0.0 if dict['throughput_time'] == 0.0 else dict['throughput_process_time'],
        'acc_throughput_running_time': 0.0 if dict['throughput_time'] == 0.0 else dict['throughput_running_time'],
        'acc_process_time': dict['process_time'], 
        'cores': dict['function_cores'],
        'success_rate': float(1- (err/thread_numb)),
        ...
        'acc_execution_total': dict['execution_end']-dict['execution_end'],
        'acc_invocation_total': dict['invocation_end']-dict['invocation_start'],
...
def run_experiment(thread_numb:int,upper_bound:int):
    ...
    while(thread_numb <= upper_bound):
        ...
        thread_numb *= 2 if not dev_mode else thread_numb + 2
