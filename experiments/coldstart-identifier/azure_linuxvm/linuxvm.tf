# create linux vm
resource "azurerm_linux_virtual_machine" "coldstart-identifier-worker" {
  depends_on = [azurerm_network_interface.coldstart-identifier-worker-ni]
  ...
  network_interface_ids = [
    azurerm_network_interface.coldstart-identifier-worker-ni.id,
  ]
...
resource "null_resource" "linux-provisioners" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.coldstart-identifier-worker,
    azurerm_public_ip.coldstart-identifier-worker-public-ip
  ]

  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    host = azurerm_linux_virtual_machine.coldstart-identifier-worker.public_ip_address
    type = "ssh"
