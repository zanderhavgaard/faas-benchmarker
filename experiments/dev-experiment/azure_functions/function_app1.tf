...
resource "azurerm_storage_blob" "dev-experiment1-code" {
  name = "dev-experiment1-function.zip"
...
resource "azurerm_function_app" "dev-experiment1" {
  ...
  app_settings = {
    HASH = data.archive_file.dev-experiment1-function-code.output_base64sha256
    WEBSITE_RUN_FROM_PACKAGE = "${azurerm_storage_blob.dev-experiment1-code.url}${data.azurerm_storage_account_sas.sas-dev-experiment.sas}"
...
resource "azurerm_template_deployment" "chageme1-function-key" {
  ...
  parameters = {
    "functionApp" = azurerm_function_app.dev-experiment1.name
  }
  resource_group_name    = azurerm_resource_group.dev-experiment-rg.name
