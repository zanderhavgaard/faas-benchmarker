...
resource "azurerm_resource_group" "dev-experiment-rg" {
  name = "dev-experiment-rg"
  location = var.azure_region
}

# create service plan for rg
resource "azurerm_app_service_plan" "dev-experiment-plan" {
  name = "dev-experiment-plan"
  location = var.azure_region
...
resource "azurerm_storage_account" "dev-experiment-experiment-storage" {
...
resource "azurerm_storage_container" "dev-experiment-container" {
  name = "dev-experiment-container"
...
data "azurerm_storage_account_sas" "sas-dev-experiment" {
  connection_string = azurerm_storage_account.dev-experiment-experiment-storage.primary_connection_string
  https_only        = false
  resource_types {
    service   = false
    container = false
...
resource "azurerm_application_insights" "dev-experiment" {
  name = "dev-experiment-insights"
  location = var.azure_region
  resource_group_name = azurerm_resource_group.dev-experiment-rg.name
