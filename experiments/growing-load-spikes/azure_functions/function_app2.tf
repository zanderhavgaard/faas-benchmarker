...
resource "azurerm_storage_blob" "growing-load-spikes2-code" {
  name = "growing-load-spikes2-function.zip"
...
resource "azurerm_function_app" "growing-load-spikes2" {
  ...
  app_settings = {
    HASH = data.archive_file.growing-load-spikes2-function-code.output_base64sha256
    WEBSITE_RUN_FROM_PACKAGE = "${azurerm_storage_blob.growing-load-spikes2-code.url}${data.azurerm_storage_account_sas.sas-growing-load-spikes.sas}"
...
resource "azurerm_template_deployment" "growing-load-spikes2-function-key" {
  depends_on = [azurerm_function_app.growing-load-spikes2]

  name = "growing-load-spikes2_get_function_key"
  parameters = {
    "functionApp" = azurerm_function_app.growing-load-spikes2.name
  }
  resource_group_name    = azurerm_resource_group.growing-load-spikes-rg.name
