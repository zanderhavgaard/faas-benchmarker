...
resource "azurerm_resource_group" "growing-load-spikes-rg" {
  name = "growing-load-spikes-rg"
  location = var.azure_region
}

# create service plan for rg
resource "azurerm_app_service_plan" "growing-load-spikes-plan" {
  name = "growing-load-spikes-plan"
  location = var.azure_region
...
resource "azurerm_storage_account" "growing-load-spikes-experiment-storage" {
...
resource "azurerm_storage_container" "growing-load-spikes-container" {
  name = "growing-load-spikes-container"
...
data "azurerm_storage_account_sas" "sas-growing-load-spikes" {
  connection_string = azurerm_storage_account.growing-load-spikes-experiment-storage.primary_connection_string
  https_only        = false
  resource_types {
    service   = false
    container = false
...
resource "azurerm_application_insights" "growing-load-spikes" {
  name = "growing-load-spikes-insights"
  location = var.azure_region
  resource_group_name = azurerm_resource_group.growing-load-spikes-rg.name
