# create linux vm
resource "azurerm_linux_virtual_machine" "growing-load-spikes-worker" {
  depends_on = [azurerm_network_interface.growing-load-spikes-worker-ni]
  ...
  network_interface_ids = [
    azurerm_network_interface.growing-load-spikes-worker-ni.id,
  ]
...
resource "null_resource" "linux-provisioners" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.growing-load-spikes-worker,
    azurerm_public_ip.growing-load-spikes-worker-public-ip
  ]

  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    host = azurerm_linux_virtual_machine.growing-load-spikes-worker.public_ip_address
    type = "ssh"
