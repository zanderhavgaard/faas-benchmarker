# create linux vm
resource "azurerm_linux_virtual_machine" "track-cloudfunctions-lifecycle-worker" {
  depends_on = [azurerm_network_interface.track-cloudfunctions-lifecycle-worker-ni]
  ...
  network_interface_ids = [
    azurerm_network_interface.track-cloudfunctions-lifecycle-worker-ni.id,
  ]
...
resource "null_resource" "linux-provisioners" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.track-cloudfunctions-lifecycle-worker,
    azurerm_public_ip.track-cloudfunctions-lifecycle-worker-public-ip
  ]

  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    host = azurerm_linux_virtual_machine.track-cloudfunctions-lifecycle-worker.public_ip_address
    type = "ssh"
