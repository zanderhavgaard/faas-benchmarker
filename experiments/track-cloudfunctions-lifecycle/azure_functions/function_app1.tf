...
resource "azurerm_storage_blob" "track-cloudfunctions-lifecycle1-code" {
  name = "track-cloudfunctions-lifecycle1-function.zip"
...
resource "azurerm_function_app" "track-cloudfunctions-lifecycle1" {
  ...
  app_settings = {
    HASH = data.archive_file.track-cloudfunctions-lifecycle1-function-code.output_base64sha256
    WEBSITE_RUN_FROM_PACKAGE = "${azurerm_storage_blob.track-cloudfunctions-lifecycle1-code.url}${data.azurerm_storage_account_sas.sas-track-cloudfunctions-lifecycle.sas}"
...
resource "azurerm_template_deployment" "track-cloudfunctions-lifecycle1-function-key" {
  depends_on = [azurerm_function_app.track-cloudfunctions-lifecycle1]

  name = "track-cloudfunctions-lifecycle1_get_function_key"
  parameters = {
    "functionApp" = azurerm_function_app.track-cloudfunctions-lifecycle1.name
  }
  resource_group_name    = azurerm_resource_group.track-cloudfunctions-lifecycle-rg.name
