...
resource "azurerm_resource_group" "weakest-link-rg" {
  name = "weakest-link-rg"
  location = var.azure_region
}

# create service plan for rg
resource "azurerm_app_service_plan" "weakest-link-plan" {
  name = "weakest-link-plan"
  location = var.azure_region
...
resource "azurerm_storage_account" "weakest-link-experiment-storage" {
...
resource "azurerm_storage_container" "weakest-link-container" {
  name = "weakest-link-container"
...
data "azurerm_storage_account_sas" "sas-weakest-link" {
  connection_string = azurerm_storage_account.weakest-link-experiment-storage.primary_connection_string
  https_only        = false
  resource_types {
    service   = false
    container = false
...
resource "azurerm_application_insights" "weakest-link" {
  name = "weakest-link-insights"
  location = var.azure_region
  resource_group_name = azurerm_resource_group.weakest-link-rg.name
