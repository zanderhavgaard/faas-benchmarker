# create linux vm
resource "azurerm_linux_virtual_machine" "weakest-link-worker" {
  depends_on = [azurerm_network_interface.weakest-link-worker-ni]
  ...
  network_interface_ids = [
    azurerm_network_interface.weakest-link-worker-ni.id,
  ]
...
resource "null_resource" "linux-provisioners" {
  ...
  depends_on = [
    azurerm_linux_virtual_machine.weakest-link-worker,
    azurerm_public_ip.weakest-link-worker-public-ip
  ]

  # setup ssh connection for provisioners
  connection {
    user = "ubuntu"
    host = azurerm_linux_virtual_machine.weakest-link-worker.public_ip_address
    type = "ssh"
