...
CREATE TABLE IF NOT EXISTS `Invocation` (
  ...
  `throughput_time` FLOAT DEFAULT 0.0 NOT NULL,
  `throughput_process_time` FLOAT DEFAULT 0.0 NOT NULL,
  `throughput_running_time` FLOAT DEFAULT 0.0 NOT NULL,
  `random_seed` INT DEFAULT -1 NOT NULL,
  `cpu` varchar(100) DEFAULT 'unknown' NOT NULL,
  `process_time` FLOAT DEFAULT 0.0 NOT NULL,
...
CREATE TABLE IF NOT EXISTS `Error` (
  ...
  `sleep` FLOAT DEFAULT 0.0 NOT NULL,
  ...
  `execution_start` FLOAT DEFAULT 0.0 NOT NULL,
  `execution_end` FLOAT DEFAULT 0.0 NOT NULL,
  `invocation_start` FLOAT DEFAULT 0.0 NOT NULL,
  `invocation_end` FLOAT DEFAULT 0.0 NOT NULL,
  FOREIGN KEY (exp_id) REFERENCES Experiment(uuid) ON DELETE CASCADE
) ENGINE=MyISAM  DEFAULT CHARSET=latin1;
