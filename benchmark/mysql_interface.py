...
class SQL_Interface:
    ...
    def get_most_recent_errors(self, args: str = '*', flag: bool = False):
        query = 'select {0} from Error where exp_id=(SELECT uuid from Experiment where id=(select max(id) from Experiment));'.format(
            args)
        res = self.tunnel.retrive_query(query)
        if(flag):
            res = np.array(res)
    ...
    def get_explicit_number_experiments(self, args: str = '*', number: int = 1, flag: bool = False, order: bool = True):
        ...
        query = 'SELECT {0} from Experiment order by id {1} limit {2};'.format(
    ...
    def get_explicit_number_coldstart(self, args: str = '*', number: int = 1, flag: bool = False, order: bool = True):
        key_word = 'desc' if order else 'asc'
        query = 'SELECT {0} from Coldstart order by id {1} limit {2};'.format(
            args, key_word, number)
        res = self.tunnel.retrive_query(query)
        return res if flag else np.array(res)
