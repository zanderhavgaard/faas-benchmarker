...
def coldtimes_graphs(directory:str, devmode:bool):
     # metadata
    directory = 'Coldtimes'
    experiment_uuids = get_uuids('linear-invocation')
    # queries
    def query_by_provider(uuid, level:int):
        return f"""select latency,instance_id, total, thread_id,  multi, provider, if(latency > coldtime, 'cold','warm') as cold, invocation_start  
    ...
    def error_by_provider(uuid):
        return f"""select count(*)/5 as number, cl_provider as provider from Error r left join Experiment e on e.uuid=r.exp_id where e.uuid = '{uuid}';"""
    ...
    # gg.bar_plot(joint_error_df,config,'error by provider for load-scenario',directory,18)
    ...
    gg.scatter_plot(joint_linear_df,config,'latency per function_instance',directory)
